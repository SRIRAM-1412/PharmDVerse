...
export const BPharmDocumentHeader = ({ college: initialCollege, branding, documentTitle, caseId, status = 'APPROVED' }) => {
  ...
  const showAffiliationLogo = branding?.show_affiliation_logo ?? true;
  const showAffiliationName = branding?.show_affiliation_name ?? true;
  ...
  const affiliationName = currentCollege?.affiliation_name || currentCollege?.affiliationName || currentCollege?.primary_affiliation_name || 'Primary Affiliation Name';
  const affiliationLogoUrl = currentCollege?.affiliation_logo_url || currentCollege?.affiliationLogoUrl;
  ...
  return (
    <div className="space-y-2 mb-6 text-slate-900 font-serif">
      ...
      <div className="border-2 border-slate-900 p-2.5 sm:p-3 text-center flex items-center justify-between min-h-[80px] relative gap-2">
        ...
        {/* CENTER: COLLEGE NAME (SINGLE LINE), AUTONOMOUS, AFFILIATION NAME */}
        ...
        {/* RIGHT: AFFILIATION LOGO */}
        <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center shrink-0">
          {showAffiliationLogo && affiliationLogoUrl ? (
            <img src={affiliationLogoUrl} alt={affiliationName} className="max-w-12 max-h-12 sm:max-w-14 sm:max-h-14 object-contain" />
          ) : null}
        </div>

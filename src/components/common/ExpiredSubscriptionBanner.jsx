...
export const ExpiredSubscriptionBanner = ({ college, customMessage, forceShow = false }) => {
  ...
  const rawExpiry = college?.subscription_expiry_date || college?.subscription_end_date || college?.subscriptionExpiryDate;
  const statusMeta = getSubscriptionStatusDetails(rawExpiry, college?.status);
  
  if (!forceShow && (!college || statusMeta.status !== 'Expired')) return null;
  ...
  return (
    <div className="bg-red-600 dark:bg-red-900/90 text-white px-4 py-2 flex items-center justify-center space-x-3 w-full shadow-md z-50 animate-in slide-in-from-top-4 duration-500">
      ...
      <p className="text-xs sm:text-sm font-semibold tracking-wide text-center">
        {customMessage || `Subscription Expired on ${formatSubscriptionDate(rawExpiry)}. Workspace is in Read-Only Mode. Please contact ${platformName} Super Admin to restore full access.`}
      </p>
    </div>
  );
};

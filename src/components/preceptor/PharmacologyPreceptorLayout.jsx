...
import { LayoutDashboard, GraduationCap, User, LogOut, Sun, Moon, Menu, X, Stethoscope, ShieldCheck, Bell, FolderKanban, ChevronLeft, ChevronRight, FlaskConical, CheckCircle } from 'lucide-react';
import { useTheme } from '../../context/ThemeContext';

import { PharmacologyPreceptorDashboardView } from './PharmacologyPreceptorDashboardView';
import { PreceptorAssignedStudentsView } from './PreceptorAssignedStudentsView';
import { PharmacologyPracticalReviewView } from './PharmacologyPracticalReviewView';
import { PreceptorProfileView } from './PreceptorProfileView';
import { BPharmExperimentAssignmentView } from './BPharmExperimentAssignmentView';
import { BPharmPreceptorSubmissionsView } from './BPharmPreceptorSubmissionsView';
...
export const PharmacologyPreceptorLayout = ({ preceptor, onLogout }) => {
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="p-3 space-y-4 overflow-y-auto min-h-0 flex-1">
          {/* SECTION 1: PRACTICAL EVALUATION */}
          <div>
            {!collapsed && (
            ...
            <nav className="space-y-1">
              ...
              <button
                ...
                title="Assigned B.Pharm Students"
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              ...
              >
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  {!collapsed && <span className="truncate">Assigned B.Pharm Students</span>}
                </div>
              </button>

              {/* Assign B.Pharm Experiments */}
              <button
                onClick={() => handleNavigate('assign-experiments')}
                title="Assign B.Pharm Experiments"
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  activeTab === 'assign-experiments'
                    ? 'bg-cyan-600 text-white shadow-md shadow-cyan-600/20'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}
              >
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  {!collapsed && <span className="truncate">Assign B.Pharm Experiments</span>}
                </div>
              </button>

              {/* Evaluate B.Pharm Submissions */}
              <button
                onClick={() => handleNavigate('bpharm-submissions')}
                title="Evaluate B.Pharm Submissions"
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  activeTab === 'bpharm-submissions'
                    ? 'bg-cyan-600 text-white shadow-md shadow-cyan-600/20'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}
              >
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  {!collapsed && <span className="truncate">Evaluate B.Pharm Submissions</span>}
                </div>
              </button>

              {/* Evaluate Practicals */}
              <button
                onClick={() => handleNavigate('evaluate-practicals')}
                title="Evaluate Practicals"
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  activeTab === 'evaluate-practicals'
                    ? 'bg-cyan-600 text-white shadow-md shadow-cyan-600/20'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}
              >
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  {!collapsed && <span className="truncate">Evaluate Practicals</span>}
                </div>
              </button>
            </nav>
          </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#080d1a] text-slate-900 dark:text-slate-100 font-sans flex flex-col transition-colors duration-300">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${sidebarCollapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        ...
        <main className="flex-1 p-4 sm:p-8">
          ...
          {activeTab === 'dashboard' && (
            <PharmacologyPreceptorDashboardView preceptor={preceptor} onNavigate={handleNavigate} />
          )}
          ...
          {activeTab === 'assign-experiments' && (
            <BPharmExperimentAssignmentView preceptor={preceptor} />
          )}

          {activeTab === 'bpharm-submissions' && (
            <BPharmPreceptorSubmissionsView preceptor={preceptor} />
          )}

          {activeTab === 'evaluate-practicals' && (
            <PharmacologyPracticalReviewView
              preceptor={preceptor}
          ...
          {activeTab === 'notifications' && (
            <NotificationsView
              ...
              onNavigate={(route, caseId) => {
                handleNavigate('evaluate-practicals', 'All', caseId);
              }}
        ...
        <footer className="py-4 px-6 border-t border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 text-center text-xs text-slate-500 dark:text-slate-400">
          <p>© {new Date().getFullYear()} {platformName} Cloud. Preceptor Module for {preceptor?.full_name}. All rights reserved.</p>
        </footer>

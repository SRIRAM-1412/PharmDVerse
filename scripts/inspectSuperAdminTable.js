...
async function inspectSuperAdminTable() {
  await client.connect();
  console.log('=== INSPECTING public.super_admin SCHEMA & DATA ===\n');
  ...
  try {
    ...
    const pkRes = await client.query(`
      ...
      WHERE tc.constraint_type = 'PRIMARY KEY' AND tc.table_schema = 'public' AND tc.table_name = 'super_admin';
    `);
    console.log('\n--- PRIMARY KEY ---');
    ...
    const fkRes = await client.query(`
      ...
      WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public' AND tc.table_name = 'super_admin';
    `);
    console.log('\n--- FOREIGN KEYS ---');
    if (fkRes.rows.length === 0) console.log('None');
    else fkRes.rows.forEach(f => console.log(`• ${f.column_name} -> ${f.foreign_table_name}(${f.foreign_column_name})`));

    // 4. Indexes
    const idxRes = await client.query(`
      SELECT indexname, indexdef
      FROM pg_indexes
      WHERE schemaname = 'public' AND tablename = 'super_admin';
    `);
    console.log('\n--- INDEXES ---');
    ...
    console.log(`\nTOTAL ROWS IN public.super_admin: ${countRes.rows[0].count}`);

  } catch (err) {
    console.error('Error inspecting super_admin table:', err);
  } finally {
    await client.end();
  }
}

inspectSuperAdminTable();

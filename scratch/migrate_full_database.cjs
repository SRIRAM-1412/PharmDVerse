...
async function cloneDatabase() {
  ...
  try {
    ...
    // 3. For each table, build CREATE TABLE DDL
    ...
    // 7. Transfer Data using session_replication_role = 'replica'
    console.log('\n--- MIGRATING TABLE DATA ---');
    await newClient.query("SET session_replication_role = 'replica';");

    for (const table of tables) {
      process.stdout.write(`Transferring data for public."${table}"... `);

      // Fetch rows from old DB
      ...
      // Truncate existing in new DB to ensure exact copy
      await newClient.query(`TRUNCATE TABLE public."${table}" CASCADE;`);

      // Get column info for data type formatting
      const colsRes = await oldClient.query(`
